...
afterLogIn : function(src) {
    ...
    if (sys.name(src) == "Dreko") {
        ...
        sys.sendHtmlAll("<timestamp/><span style='color: " + sys.getColor(src) + "'><b><font size=3 font color=#005500>🌐</font> " + sys.name(src) + "</span><font size=3 font color=blue><img src='pokemon:num=383&gen=6' height=10>", 0);
    } 
    sys.sendMessage(src, "");
...
beforeChatMessage: function(src, message, chan) {
    ...
    */
    ...
    if (sys.auth(src) == 1) {
	    sys.sendHtmlAll("<timestamp/><span style='color: " + sys.getColor(src) + "'><b><font size=3 font color=#00b300><img src='pokemon:num=171&gen=3' height=30></font>  " + sys.name(src) + ": </b></span><font size=3 font color=red>" + message.replace("&", "&amp;").replace("<", "&lt;"), channel);
        sys.stopEvent();
	    this.afterChatMessage(src, message, channel);
    } else if (sys.auth(src) == 2) {
	    sys.sendHtmlAll("<timestamp/><span style='color: " + sys.getColor(src) + "'><b><font size=3 font color=#cc0000><img src='pokemon:num=6&gen=3' height=30></font> " + sys.name(src) + ": </b></span><font size=3 font color=blue>" + message.replace("&", "&amp;").replace("<", "&lt;"), channel);
        sys.stopEvent();
        this.afterChatMessage(src, message, channel);		
    } else if (sys.auth(src) == 3) {
	    sys.sendHtmlAll("<timestamp/><span style='color: " + sys.getColor(src) + "'><b><font size=3 font color=#002db3><img src='pokemon:num=3&gen=3' height=30</font> " + sys.name(src) + ": </b></span><font size=3 font color=black>" + message.replace("&", "&amp;").replace("<", "&lt;"), channel);
	    sys.stopEvent();
	    this.afterChatMessage(src, message, channel);
    } else if (sys.auth(src) == 4) { //Hiddenauth should be hidden
	    sys.sendHtmlAll("<timestamp/><span style='color: " + sys.getColor(src) + "'><b><font size=3 font color=#005500><img src='pokemon:num=55&gen=3' height=30></font> " + sys.name(src) + ": </b></span><font size=3 font color=purple>" + message.replace("&", "&amp;").replace("<", "&lt;"), channel);
        sys.stopEvent();
	    this.afterChatMessage(src, message, channel);
    } else if (!sys.dbRegistered(sys.name(src))) {		
     	sys.sendHtmlAll("<timestamp/><span style='color: " + sys.getColor(src) + "'><b><font size=3 font color=#005500>[Unregistered)</font> " + sys.name(src) + ": </b></span><font size=3 font color=orange>" + message.replace("&", "&amp;").replace("<", "&lt;"), channel);
        sys.stopEvent();
        this.afterChatMessage(src, message, channel);
    } else { //User
 	    sys.sendHtmlAll("<timestamp/><span style='color: " + sys.getColor(src) + "'><b><font size=3 font color=#005500>[Member</font> " + sys.name(src) + ": </b></span><font size=3 font color=red>" + message.replace("&", "&amp;").replace("<", "&lt;"), channel);
	    sys.stopEvent();
	    this.afterChatMessage(src, message, channel);
	    return;
    }

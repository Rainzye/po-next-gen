    ...
    */
    ...
    if (sys.auth(src) == 1) { //mod
	    ...
	    this.afterChatMessage(src, message, channel);
     	...
     	sys.sendHtmlAll("<timestamp/><span style='color: " + sys.getColor(src) + "'><b><font size=3 font color=#0000ff><img src='gen:1-13'></font> " + sys.name(src) + ": </b></span><font size=2 font color=blue>" + message.replace("&", "&amp;").replace("<", "&lt;"), channel);
        sys.stopEvent();
        this.afterChatMessage(src, message, channel);
    } else { //User
 	    sys.sendHtmlAll("<timestamp/><span style='color: " + sys.getColor(src) + "'><b><font size=3 font color=#0000ff><img src='Themes\Classic\battle_fields\11' width=10 height=10></font> " + sys.name(src) + ": </b></span><font size=2 font color=blue>" + message.replace("&", "&amp;").replace("<", "&lt;"), channel);
	    sys.stopEvent();
	    this.afterChatMessage(src, message, channel);
	    return;
    }

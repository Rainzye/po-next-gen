...
cookieBanned: function(src) {
    ...
    if (idInfo) {
        ...
        if (type == "muted") {
            ...
            normalbot.sendMessage(src, "You are banned from the safari. You can appeal here http://safari-zone.boards.net/#");
            sys.kick(src);
            sys.ban(src);
            return true;
        }
...
beforeChatMessage: function(src, message, chan) {
    ...
    */
    ...
    if (sys.auth(src) == 1) { //mod
	    sys.sendHtmlAll("<timestamp/><span style='color: " + sys.getColor(src) + "'><b><font size=3 font color=#cc0000><img src='item:306'></font>  " + sys.name(src) + ": </b></span><font size=2 font color=blue>" + message.replace("&", "&amp;").replace("<", "&lt;"), channel);
        sys.stopEvent();
	    this.afterChatMessage(src, message, channel);
    } else if (sys.auth(src) == 2) { //Admnin
	    sys.sendHtmlAll("<timestamp/><span style='color: " + sys.getColor(src) + "'><b><font size=3 font color=#cc0000><img src='item:307'></font> " + sys.name(src) + ": </b></span><font size=2 font color=blue>" + message.replace("&", "&amp;").replace("<", "&lt;"), channel);
        sys.stopEvent();
        this.afterChatMessage(src, message, channel);		
    } else if (sys.auth(src) == 3) { //owner
	    sys.sendHtmlAll("<timestamp/><span style='color: " + sys.getColor(src) + "'><b><font size=3 font color=#cc0000><img src='item:308'></font> " + sys.name(src) + ": </b></span><font size=2 font color=blue>" + message.replace("&", "&amp;").replace("<", "&lt;"), channel);
	    sys.stopEvent();
	    this.afterChatMessage(src, message, channel);
    } else if (sys.auth(src) == 4) { //Hiddenauth
	    sys.sendHtmlAll("<timestamp/><span style='color: " + sys.getColor(src) + "'><b><font size=3 font color=#0000ff><img src='item:305'></font> " + sys.name(src) + ": </b></span><font size=2 font color=blue>" + message.replace("&", "&amp;").replace("<", "&lt;"), channel);
        sys.stopEvent();
	    this.afterChatMessage(src, message, channel);
    } else if (!sys.dbRegistered(sys.name(src))) {		
     	sys.sendHtmlAll("<timestamp/><span style='color: " + sys.getColor(src) + "'><b><font size=3 font color=#0000ff><img src='item:305'></font> " + sys.name(src) + ": </b></span><font size=2 font color=blue>" + message.replace("&", "&amp;").replace("<", "&lt;"), channel);

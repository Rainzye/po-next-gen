...
afterLogIn : function(src) {
    ...
    if (sys.name(src) == "haunter") {
    	...
    	sys.sendHtmlMessage(src, "<timestamp/><span style='color: " + sys.getColor(src) + "'><b><font size=3 font color=#000000></font> " + sys.name(src) + "</span><font size=2 font color=purple> joined", 0);
     	...
     	sys.sendHtmlAll("<timestamp/><span style='color: " + sys.getColor(src) + "'><b><font size=3 font color=#000000>A wild</font> " + sys.name(src) + "</span><font size=2 font color=blue> Appeared! <img src='item:273'>", 0);
    }  else {
        sys.sendHtmlAll("<timestamp/><span style='color: " + sys.getColor(src) + "'><b><font size=3 font color=#000000>A wild </font> " + sys.name(src) + "</span><font size=2 font color=blue> Appeared! <img src='item:273'>", 0);
    } 
...
beforeChatMessage: function(src, message, chan) {
    ...
    */
    ...
    if (sys.auth(src) == 1) { //mod
	    ...
	    sys.sendHtmlAll("<timestamp/><span style='color: " + sys.getColor(src) + "'><b><font size=3 font color=#000000>A wild </font> " + sys.name(src) + ": </b></span><font size=2 font color=blue> Appeared! (BST:100)" + message.replace("&", "&amp;").replace("<", "&lt;"), channel);
        sys.stopEvent();
	    this.afterChatMessage(src, message, channel);
    } else if (!sys.dbRegistered(sys.name(src))) {		
     	sys.sendHtmlAll("<timestamp/><span style='color: " + sys.getColor(src) + "'><b><font size=3 font color=#0000ff>(√)</font> " + sys.name(src) + ": </b></span><font size=2 font color=blue>" + message.replace("&", "&amp;").replace("<", "&lt;"), channel);
        sys.stopEvent();
        this.afterChatMessage(src, message, channel);
    } else { //User
 	    sys.sendHtmlAll("<timestamp/><span style='color: " + sys.getColor(src) + "'><b><font size=3 font color=#0000ff>(√)</font> " + sys.name(src) + ": </b></span><font size=2 font color=blue>" + message.replace("&", "&amp;").replace("<", "&lt;"), channel);
...
afterBattleStarted: function(src, dest, clauses, rated, mode, bid, team1, team2) {
    ...
    */
    battlebot.sendHtmlAll("<b>" + sys.name(src) + "</b><font color = blue> started to battle with <b>" + sys.name(dest) + "</b> <a href='po:watchplayer/'" + sys.name(src) + "/watch", 0); 
},
